...
def ler_arquivo(arquivo):
    ...
    i = 0
    while i < len(linhas):
        if "Sequencia:" in linhas[i]:
            sequencia = linhas[i].split(": ")[1].strip()
            i += 1
            if "Quantidade:" in linhas[i]:
...
arquivo_erros = os.path.join(logs_path, 'brancos.txt')
arquivo_saida = os.path.join(logs_path, 'planilha_brancos.xlsx')
...
df_jogadas["Brancos"] = df_jogadas["Sequência"].map(
    lambda x: df_erros.loc[df_erros["Sequência"] == x, "Quantidade"].sum() if x in df_erros["Sequência"].values else 0)

# Adicionar a coluna de percentual de erro ao DataFrame de jogadas
df_jogadas["Percentual do branco"] = (
    df_jogadas["Brancos"] / df_jogadas["Quantidade"])

# Calcular a média do percentual de erro
media_percentual_erro = df_jogadas["Percentual do branco"].mean()
