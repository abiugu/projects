...
def gerar_planilha_excel(padroes_analise, caminho_arquivo_excel):
    ...
    header = ["Percentual Atual", "Comparação 25", "Comparação 50", "Comparação 100", "Comparação 500", "Acertos", "Erros", "Jogadas", "Últimos Erros Consecutivos", "Máx. Erros Consecutivos", "Assertividade Acertos (%)"]
    sheet.append(header)
    ...
    for chave_padrao, dados in padroes_analise.items():
        ...
        row = [percentual_atual, comp_25, comp_50, comp_100, comp_500, acertos, erros, jogadas, erros_consecutivos, max_erros_consecutivos, f"{assertividade_acertos:.2f}%"]
        sheet.append(row)
