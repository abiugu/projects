...
def analisar_padroes(resultados, porcentagens_25, porcentagens_50, porcentagens_100, porcentagens_500):
    ...
    for i in range(len(resultados) - 2):
        ...
        if resultados[i][0] == resultados[i][1] == resultados[i][2]:
            ...
            acerto_branco = resultados[i + 1][0] != cor_atual or resultados[i + 2][0] != cor_atual
...
def gerar_planilha_excel(padroes_analise, caminho_arquivo_excel):
    ...
    header = ["Cor Atual", "Percentual Atual", "Comparação 25", "Comparação 50", "Comparação 100", "Comparação 500", "Acertos", "Acertos Branco", "Erros", "Jogadas", "Últimos Erros Consecutivos", "Máx. Erros Consecutivos", "Assertividade Acertos (%)", "Assertividade Acertos Branco (%)", "Assertividade Total (%)"]
    sheet.append(header)
...
def main():
    ...
    caminho_arquivo_excel = os.path.join(desktop_path, 'LOGS', 'Dados log global cor gale.xlsx')
